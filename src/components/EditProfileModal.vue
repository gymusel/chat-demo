<template>
  <div class="flex flex-col p-6">
    ...
    <form @submit.prevent="updateProfile">
      <div class="mb-2">
        <input type="displayName" v-model="displayName" placeholder="ユーザーネーム" class="text-xl w-4/5 p-3 border rounded" />
      </div>
      <div class="mb-2">
        <input type="language" v-model="language" placeholder="話せる言語" class="text-xl w-4/5 p-3 border rounded" />
      </div>
      <div class="mb-2">
        <input type="country" v-model="country" placeholder="出身国"  class="text-xl w-4/5 p-3 border rounded"/>
      </div>
      <div class="mb-2">
      ...
      <button type="submit" class="text-xl w-3/5 bg-green-800 text-white py-2 rounded">登録</button>
    </form>

    <button @click="signOut" class="text-xl w-3/5 bg-red-800 text-white py-2 rounded mx-auto my-10">サインアウト</button>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    updateProfile() {
      ...
      this.$store.commit("updateUser", this.displayName, this.language, this.country)
